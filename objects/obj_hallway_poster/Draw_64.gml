// obj_hallway_poster — Draw GUI Event
// Window shape mirrors the poster: width = 2x displayed sprite width,
// height = sprite height + button row + text strip.
...
// ── 1. Scale poster sprite ────────────────────────────────────────────────────
// For a very tall portrait image (e.g. 1536x2752) the height cap drives scale.
var _sw = sprite_get_width(spr_hallway_poster);
var _sh = sprite_get_height(spr_hallway_poster);
var _max_spr_h = 210;
...
// ── 2. Text metrics ───────────────────────────────────────────────────────────
var _cap_str  = "Sounds like useful information for quizzes.";
...
// ── 3. Window dimensions ─────────────────────────────────────────────────────
// Width  = 2x the sprite display width  (window frames the poster)
// Height = button row + sprite + gap + caption + gap + hint + bottom pad
var _btn_row = 24;
var _gap     = 8;
...
// ── 5. Close button (X) — top-right corner ───────────────────────────────────
var _btn_sz = 18;
...
// ── 6. Poster sprite (centred in its allocated row) ───────────────────────────
var _spr_area_y = _fy1 + _btn_row;
var _spr_cx     = _cx;
var _spr_cy     = _spr_area_y + _disp_h * 0.5;

// draw_sprite_ext places the ORIGIN at (x,y). Offset so the visual centre
// lands on (_spr_cx, _spr_cy).
var _spr_draw_x = _spr_cx - _sw * _scale * 0.5
                + sprite_get_xoffset(spr_hallway_poster) * _scale;
var _spr_draw_y = _spr_cy - _sh * _scale * 0.5
                + sprite_get_yoffset(spr_hallway_poster) * _scale;

draw_sprite_ext(spr_hallway_poster, 0,
                _spr_draw_x, _spr_draw_y,
                _scale, _scale, 0, c_white, 1);

// ── 7. Caption — word-wraps within inner width ────────────────────────────────
